...
The scraper consists of three main scripts that work together:
...
Finally, scrape detailed information for tournaments:
...
| `--output` | | | Output JSON file (auto-generated from year/month if not specified) |
| `--rate-limit` | | `1.0` | Initial requests per second |
...
# Lower rate limit for more conservative scraping
python src/scraper/get_tournament_details.py --year 2025 --month 1 --rate-limit 1.0
...
**Rate Limiting:**
- Uses fixed rate limiting with token bucket algorithm
- Maintains constant rate as specified (default 1.0 req/s)
- Prevents overwhelming the FIDE server
- Allows small bursts up to 2x the rate limit
...
**`get_tournament_details.py` specific error handling:**
